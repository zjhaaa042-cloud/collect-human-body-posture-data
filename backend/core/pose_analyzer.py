...
_MPL_CACHE = _PROJECT_ROOT / "temp" / "matplotlib"
...
def _quiet_native_stderr():
    """MediaPipe 0.10.35 emits unavoidable telemetry errors on offline PCs."""
    saved_fd = None
    null_fd = None
    try:
        saved_fd = os.dup(2)
...
class PoseAnalyzer:
    ...
    def submit(self, rgb_frame: np.ndarray, timestamp_ms: Optional[int] = None, force: bool = False):
        ...
        with self._lock:
            self._pending = (np.ascontiguousarray(rgb_frame), timestamp_ms)
        self._event.set()
        return timestamp_ms
