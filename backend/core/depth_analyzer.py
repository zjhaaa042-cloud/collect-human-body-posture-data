...
class DepthAnalyzer:
    ...
    def analyze_distance(
        ...
        """Serialize stateful pose, mask and stability analysis across callers."""
        with self._analysis_lock:
            return self._analyze_distance_unlocked(
                depth_frame,
                color_frame,
                depth_scale,
                wait_for_pose,
    ...
    def _analyze_distance_unlocked(
        self,
        depth_frame: np.ndarray,
        color_frame: np.ndarray = None,
        depth_scale: float = 1.0,
    ...
    def detect_human(self, depth_frame, depth_scale=1.0, color_frame=None):
        ...
        with self._analysis_lock:
            bbox = None if self._last_bbox is None else tuple(self._last_bbox)
        return info.status not in {DistanceStatus.NO_DATA, DistanceStatus.NO_HUMAN}, bbox
    ...
    def configure_distance_window(
        ...
        """Atomically switch the condition-specific distance window."""
        with self._analysis_lock:
            values = tuple(
                float(value)
                for value in (
