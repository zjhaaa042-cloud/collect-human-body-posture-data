...
7. M01–M13 的软件门禁已经实现，但人体测量工具尚未到位，因此当前不能完成合格真实受试者数据的最终闭环。
...
9. 当前自动验证结果为后端/协议测试 72 项通过、前端测试 9 项通过、前端生产构建通过；这些结果证明软件基线可进入正式场地验收和 Pilot，不替代真实受试者验收。
...
| M01–M23 软件录入与校验 | 已实现 | M01–M13 仍受实物工具门禁 |
...
- M01–M13 必填，且每个定义中的每个 `field_name` 都必须完成；
- M14–M23 可整项为空，整项为空时前端不发送该项目；
...
四个工具编号都必须是有效的 1–64 位标识，并确认已经完成零点或校准状态检查。软件门禁已经生效；实物工具未到位仍是当前真实采集阻塞项。
...
6. M01–M13 全部字段完成，人体测量 revision 存在且哈希一致；
...
同时完成零点/校准检查、操作员 SOP 演练和一名受试者的 M01–M13 全字段复测验收。
...
- M01–M13 与四类工具编号、设备检查确认共同构成完成门禁；
