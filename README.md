...
- 默认双机流程的 Gemini 距离提示跟随登记时的会话默认距离（建议 `2.5 m`）；未填写时，距离提示使用 `2.5 m`，记录仍保留为空。临时修改单角度距离时，以输入的毫米值人工对位并留痕。Gemini 需确保头、手和脚完整入镜；D435i 尽量覆盖躯干、腰臀并保持稳定，不为追求全身入镜而单独改变同组站位。
