...
> 默认工作台协议：`dual-rgbd-v2.0`（双机八角度）；`RealAnthro-RGBD-v1.0` 与旧 `data/sessions` 格式保留兼容。项目主要面向 Windows 下 Gemini 336L 与 D435i 实机采集。

## 功能特性

- 默认双机流程为 360° 每 45° 一组，共 8 组；每组由 Gemini 336L 与 D435i 各采 5 帧五模态数据。
...
1. 在“摄像头连接”中依次连接 Gemini 336L 和 D435i。两台都连接后，实时预览自动切换为双画面。
2. 进入默认的“**双机 8 角度**”页，填写匿名编号（`S0001` 格式）、选择任意可写 Windows 文件夹；服装仅作可选备注。
3. 设定一个可选的默认距离；每个角度前都可以按实际站位单独修改，不设强制距离标准。
4. 按 `0°、45°、90°、135°、180°、225°、270°、315°` 顺时针依次转身。确认全身入镜、画面稳定后，点击“双机同步采集此角度”。
5. 系统等待 2 秒后并发抓取两路各 5 帧，记录每对帧的主机时间差；成功后自动进入下一角度。
6. 八组完成后，数据位于所选文件夹的 `body_posture_dual_v2/subjects/<subject_id>/`。每帧保存正确色彩的 RGB、原始深度、伪彩深度和带 RGB 颜色的 PLY 点云；不再启用或保存左右红外。
