...
def stop_process_tree(
    ...
    *,
    graceful_wait: float = 0.0,
    signal_wait: float = 10.0,
):
    ...
    if os.name == "nt":
        ...
        try:
            ...
            return
    try:
        process.wait(timeout=signal_wait)
        return
    except subprocess.TimeoutExpired:
        pass
...
def main() -> int:
    ...
    try:
        ...
        # Closing Vite disconnects the browser. The backend then waits for any
        # active capture/commit to finish before its idle-shutdown task exits.
        stop_process_tree(backend, graceful_wait=130.0)
